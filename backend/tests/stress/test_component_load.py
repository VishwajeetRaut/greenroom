...
def test_evaluation_survives_dead_primary_without_fallback(monkeypatch):
    ...
    """
    # Both constructors — evaluation runs on Azure OpenAI, so patching
    # _make_llm alone would leave the primary alive and never reach the
    # fallback path this test exists to exercise.
    monkeypatch.setattr(llm_mod, "_make_llm", lambda *a, **kw: _DeadModel())
    monkeypatch.setattr(llm_mod, "_make_azure_llm", lambda *a, **kw: _DeadModel())
