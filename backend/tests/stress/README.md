...
process-wide stores they share (`session_store`, `rate_limit`).
...
| Test | Bug | Status |
|---|---|---|
| `test_postgres_limiter_enforces_limit_under_burst` | `_check_postgres` counts then inserts non-atomically, so a burst is admitted whole — no rate limiting under the exact condition it exists for. | still xfail |
| `test_evaluation_survives_dead_primary_without_fallback` | `_fallback_chat`'s `RuntimeError` escaped `evaluate_session` when no fallback is configured, so a Groq 429 became a 500 instead of the last-resort report. | fixed; now a regression test |
