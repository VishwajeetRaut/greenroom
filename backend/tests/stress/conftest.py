...
def fake_llm(monkeypatch):
    ...
    def install(responses: list[str]):
        ...
        # Both constructors: the live conversation runs on Groq via _make_llm,
        # while evaluation (evaluate_session, _self_critique,
        # evaluate_diagram) moved to Azure OpenAI via _make_azure_llm.
        # Patching only the first lets the evaluation tests fall through
        # to a real provider and fail on missing credentials rather than
        # on the behaviour they mean to exercise.
        monkeypatch.setattr(llm_mod, "_make_llm", lambda *a, **kw: fake)
        monkeypatch.setattr(llm_mod, "_make_azure_llm", lambda *a, **kw: fake)
        return fake
